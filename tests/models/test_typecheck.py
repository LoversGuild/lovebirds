...
class TestTypecheckPersonRef:
    ...
    def test_a_dataclass_of_the_wrong_class_is_a_type_error(self) -> None:
        """typeguard tries the members of a union in turn and moves on to the
        next only on TypeCheckError. A PersonRefByName checked against
        PersonRefById has no `id`; if that surfaced as AttributeError, the
        whole PersonRef check would fail on the first member instead of going
        on to the one that matches.
        """
        with pytest.raises(TypeCheckError):
            check_type(PersonRefByName(name="Alice Smith"), PersonRefById)
